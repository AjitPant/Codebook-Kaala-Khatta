...
bool properInter(p3 a, p3 b, p3 c, p3 d, p3 &out) {
  ...
  int oa = geometry_2d::sgn(cd | a), ob = geometry_2d::sgn(cd | b),
      oc = geometry_2d::sgn(ab | c), od = geometry_2d::sgn(ab | d);
...
int windingNumber3D(vector<vector<p3>> fs) {
  double sum = 0;
  for (vector<p3> f : fs) sum += remainder(areaOnSphere(1, f), 4 * M_PI);
